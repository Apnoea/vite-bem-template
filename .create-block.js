...
fs.mkdir(`${blocksDir}/${blockName}`, () => {
  let error = false
  if (fs.existsSync(`${blocksDir}/${blockName}/${blockName}.pug`)) {
    error = true
    console.log('Файл шаблона уже существует')
  } else {
    fs.writeFileSync(`${blocksDir}/${blockName}/${blockName}.pug`,
  ...
  if (fs.existsSync(`${blocksDir}/${blockName}/${blockName}.scss`)) {
    error = true
    console.log('Файл стилей уже существует')
  } else {
    fs.writeFileSync(`${blocksDir}/${blockName}/${blockName}.scss`, `.${blockName} {\n  position: relative;\n}\n`)
  }
  if (error === false) {
